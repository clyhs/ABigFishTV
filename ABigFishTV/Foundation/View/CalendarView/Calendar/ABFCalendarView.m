...
@interface ABFCalendarView()<UIScrollViewDelegate,ABFCalendarDateItemDelegate>
...
- (void)resetDayItemsFrame:(NSArray *)dayItems
{
    ...
    for (NSInteger i = 0; i < [dayItems count]; i++)
    {
        ...
        ABFCalendarDateItem *dayItem = dayItems[i];
        dayItem.frame = frame;
    }
}
...
- (NSArray *)getDayItemArray
{
    ...
    for (NSInteger i = 0; i < dayItemCount; i++)
    {
        ABFCalendarDateItem *dayItem = [ABFCalendarDateItem new];
...
- (void)setupDayItemArray:(NSArray *)dayItemArray withDataArray:(NSArray *)dataArray
{
    ...
    for (NSInteger i = 0; i < dayItemCount; i++)
    {
        ABFCalendarDateItem *dayItem = dayItemArray[i];
        ABFDateItemModel *model = dataArray[i];
...
- (NSArray *)getDataArrayWithDate:(NSDate *)date
{
    ...
    for (NSInteger i = 0; i < numberOfDaysInLastMonth; i++)
    {
        ...
        ABFDateItemModel *model = [self getDayItemModelWithDate:date isCurrentMonth:NO];
    ...
    for (NSInteger i = 0; i < numberOfDaysInMonth; i++)
    {
        ...
        ABFDateItemModel *model = [self getDayItemModelWithDate:date isCurrentMonth:YES];
        [dataArray addObject:model];
    }
    ...
    if ([self compareDate1:date date2:self.selectedDate])
    {
        ...
        ABFDateItemModel *model = dataArray[selectedIndex];
        model.selected = YES;
    }
...
- (ABFDateItemModel *)getDayItemModelWithDate:(NSDate *)date isCurrentMonth:(BOOL)isCurrentMonth
{
    ABFDateItemModel *model = [ABFDateItemModel new];
...
#pragma mark - ABFCalendarDateItemDelegate
